...
import com.gecq.musicwave.database.provider.FavoritesStore.FavoriteColumns;
import com.gecq.musicwave.database.provider.RecentStore;
import com.gecq.musicwave.loaders.FavoritesLoader;
import com.gecq.musicwave.loaders.LastAddedLoader;
...
public final class MusicUtils {
    ...
    public static IMusicWaveService mService = null;
    ...
    public static final ServiceToken bindToService(final Context context,
        ...
        contextWrapper.startService(new Intent(contextWrapper, PlayerService.class));
        final ServiceBinder binder = new ServiceBinder(callback);
        if (contextWrapper.bindService(
                new Intent().setClass(contextWrapper, PlayerService.class), binder, 0)) {
     ...
     *       PlayerService.#openCurrentAndNext()} is used, the user won't
     *       be able to travel to the previously skipped track. To remedy this,
     *       {@link PlayerService.#openCurrent()} is called in {@link
     *       PlayerService.#prev()}. {@code #startService(Intent intent)}
     *       is called here to specifically invoke the onStartCommand used by
     *       {@link PlayerService}, which states if the current position
    ...
    public static void previous(final Context context) {
        final Intent previous = new Intent(context, PlayerService.class);
        previous.setAction(PlayerService.PREVIOUS_ACTION);
    ...
    public static void playNext(final long[] list) {
        ...
        try {
            mService.enqueue(list, PlayerService.NEXT);
        } catch (final RemoteException ignored) {
        }
    }
...
//        AppMsg.makeText((Activity)context, message, AppMsg.STYLE_CONFIRM).show();
    ...
    public static void addToQueue(final Context context, final long[] list) {
        ...
        try {
            mService.enqueue(list, PlayerService.LAST);
...
//    }
    ...
    public static void notifyForegroundStateChanged(final Context context, boolean inForeground) {
        ...
        if (old == 0 || sForegroundActivities == 0) {
            ...
            intent.setAction(PlayerService.FOREGROUND_STATE_CHANGED);
            intent.putExtra(PlayerService.NOW_IN_FOREGROUND, sForegroundActivities != 0);
